...
外部CSSファイルを読み込む前に、次の共通基礎を確認してください。
...
コメントの考え方は共通基礎で確認済みです。CSSでコメントを書く場合は、`/* */` で囲みます：
...
<Hint>
コードを書く前に、問題文のHTMLで指定を当てたい要素を先に確認しましょう。`h1`・`p`・`index.html` は、対象を決めたあとに必要なタグ・セレクタ・値として入れると迷いにくくなります。
</Hint>
...
<Hint>
コードを書く前に、問題文のHTMLで指定を当てたい要素を先に確認しましょう。`<h2>`・`<ul>`・`<li>` は、対象を決めたあとに必要なタグ・セレクタ・値として入れると迷いにくくなります。
</Hint>
...
<iframe
  width="560"
  height="315"
  src="https://www.youtube.com/embed/WW0En8ClkkM"
...
<Hint>
コードを書く前に、問題文のHTMLで指定を当てたい要素を先に確認しましょう。`h1`・`#800080`・`h2` は、対象を決めたあとに必要なタグ・セレクタ・値として入れると迷いにくくなります。
</Hint>
...
<Hint>
コードを書く前に、問題文のHTMLで指定を当てたい要素を先に確認しましょう。`#FF6B35`・`rgb(46, 125, 50)`・`rgba(63, 81, 181, 0.6)` は、対象を決めたあとに必要なタグ・セレクタ・値として入れると迷いにくくなります。
</Hint>
...
<Hint>
コードを書く前に、問題文のHTMLで指定を当てたい要素を先に確認しましょう。`index.html`・`style.css` は、対象を決めたあとに必要なタグ・セレクタ・値として入れると迷いにくくなります。
</Hint>
